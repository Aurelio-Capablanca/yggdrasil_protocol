...
impl Expression{
    ...
    pub fn as_numbers(&self) -> f64{
        match self {
            Expression::Number(n, b ) => {println!("You hit here! NUMBERS as_numbers {}",*n); *n },
    ...
    pub fn to_numeric(&self) -> Expression{
        match self {
            Expression::Boolean(b) => { println!("You hit here! to_numeric "); Expression::Number(if *b { 1.0 } else { 0.0 }, 10) },
            _=> self.clone()
        }
    }
    ...
    pub fn to_boolean(&self) -> Expression {
        match self {
            Expression::Number(n, b) => Expression::Boolean(*n != 0.0),
            _=> self.clone()
        }
    }
}